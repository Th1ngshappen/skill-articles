...
class MarkdownContentView @JvmOverloads constructor(
        ...
        children.forEach {
            if (it is MarkdownTextView) {
                it.layout(
                    left - paddingLeft / 2,
                    usedHeight,
                    r - paddingRight / 2, // 6: 2:00:18 (здесь подойдёт полная родительская ширина r)
                    usedHeight + it.measuredHeight
                )
            } else {
    ...
    fun renderSearchResult(searchResult: List<Pair<Int, Int>>) {
        ...
        // groupByBounds возвращает список, который в себе содержит коллекции пар
    ...
    // контейнер для сохранения ids и состояния дочерних представлений
